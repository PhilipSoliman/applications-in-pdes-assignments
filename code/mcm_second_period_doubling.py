...
period_doubling_idx = 2
with open(data_dir / "mcm_limit_cycles.json", "r") as f:
    limit_cycles = json.load(f)
    solutions = limit_cycles["points"]
    if len(solutions) > period_doubling_idx:
        print("removing previous run")
        limit_cycles["points"] = [solutions[0:period_doubling_idx]]
        limit_cycles["parameter"] = [limit_cycles["parameter"][0:period_doubling_idx]]
