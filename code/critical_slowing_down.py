...
def time_integrator_with_noise(modelfun, params, x0, dt, T, n_perturbs=10):
    t = np.arange(0, T, dt)
    x = np.zeros((len(x0), len(t)))
    x[:, 0] = x0
    p_interval = int(len(t)/n_perturbs)
    ...
    for i in range(1,len(t)):
        ...
        # if i % p_interval == 0:
        #     scale = 0.05 #*np.linalg.norm(dx) # scale noise with size of dx
        #     x[:-1, i] += np.random.normal(0, scale, len(x0)-1) # add noise to Rac and rho (not B)

        # add constant perturbation
        if i % p_interval == 0:
            scale = 0.05 #*np.linalg.norm(dx) # scale noise with size of dx
            x[:-1, i] += np.array([scale, scale])
...
ic = (0.0, 0.0, B0)
t, y_tim = time_integrator_with_noise(Fun_ext, params_ext, ic, dt, T)
